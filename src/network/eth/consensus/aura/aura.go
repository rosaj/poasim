...
// Aura is the proof-of-authority consensus engine
type Aura struct {
	name string
...
func New(name string, config *params.AuraConfig, db ethdb.Database) *Aura {
...
func (a *Aura) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if number > 0 {
		if header.Difficulty == nil || header.Difficulty.Cmp(chain.Config().Aura.Difficulty) != 0 {
...
func (a *Aura) verifySeal(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if !a.inTurn(signer, header.Time) {
...
func (a *Aura) inTurn(signer common.Address, time uint64) bool {



	step := time / a.config.Period
	authNum := uint64(len(a.config.Authorities))
	turn := step % authNum


	// ako je vrijeme djeljivo sa periodom onda zapravo authority moze potpisat taj blok
	//
	if time % a.config.Period == 0 {
		// ako smo na prvom authority-u preskoci na zadnjeg
		if turn == 0 {
			turn = authNum
		}
		// blok zapravo potpisuje prethodni authority
		// npr za period 15 i vrijeme 15 ispada 15/15 = 1, 1 % authNum = 1, a treba biti 0
		turn -= 1
	}
...
func (a *Aura) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	if now := SecondsNow(); header.Time < now {
		header.Time = now
	}
	return nil
}
...
func (a *Aura) Seal(chain consensus.ChainReader, block *types.Block, results func(block *types.Block)) error {
	header := block.Header()
	...
	if !a.inTurn(a.signer, time) {
		// not authorized to sign
		return errUnauthorized
	}

	// Sweet, the protocol permits us to sign the block, wait for our time
	delay := ToDuration(float64(header.Time)) - ToDuration(godes.GetSystemTime())

	// Sign all the things!
	sighash, err := signFn(accounts.Account{Address: gcommon.Address(signer)}, AuraRLP(header))
	if err != nil {
		return err
	}
	...
	a.log("Sealed block", block.NumberU64())
	//Print(a.name, "Sealed", block.NumberU64(), "after", delay)
...
// AuraRLP returns the hash which is used as input for the proof-of-authority
...
// Close implements consensus.Engine. It's a noop for Aura as there is are no background threads.
...
func (a *Aura) log(p ...interface{})  {
	if LogConfig.LogConsensus {
		Print("Aura", a.name, p)
	}
}
